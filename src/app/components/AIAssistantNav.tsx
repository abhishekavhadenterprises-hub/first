...
export function AIAssistantNav() {
  ...
  return (
    <div className={`w-full px-6 py-3 fixed left-0 right-0 z-40 transition-all duration-300 ease-in-out ${isMainNavDropdownOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex items-center gap-2 px-3 py-3 text-sm font-medium transition-colors border-b-2 whitespace-nowrap ${isActive
